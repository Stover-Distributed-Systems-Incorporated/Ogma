...
1. [Download **Ogma.pkg**](https://github.com/Stover-Distributed-Systems-Incorporated/Ogma/releases/latest/download/Ogma.pkg) and double-click it
...
> **Prefer to build from source?** [Download the source zip](https://github.com/Stover-Distributed-Systems-Incorporated/Ogma/releases/latest/download/ogma.zip), unzip, and double-click `install.command` — it compiles the app on your machine (requires Xcode Command Line Tools, which it offers to install).
...
If the local speech service fails, Ogma stops recording and reports the error.
When a partial transcript is available, it opens for review—even with automatic
insertion enabled—so you can recover the recognized text. Partial recovery does
not invoke Intent Rewrite. Cancel discards the session and stops pending or paced
insertion; characters already inserted remain in the target application.

Read-aloud and speed-reading shortcuts require a fresh selection copied by the
active application. If Copy does not produce text, Ogma asks you to select text
instead of reading previous clipboard contents.
...
Download and run the [latest `Ogma.pkg`](https://github.com/Stover-Distributed-Systems-Incorporated/Ogma/releases/latest/download/Ogma.pkg) — the app re-syncs its helper scripts automatically on first launch after an update. (Source installs: re-run `install.command`.)
...
```bash
# Existing suite; skips model inference, Swift compilation, and audio-device playback.
OGMA_SKIP_BOOTSTRAP=1 bash macos/tests/test.sh --fast

# Model-free daemon, shell, and native Swift regressions.
python3 -m unittest discover -s macos/tests -p 'test_*.py' -v
```

The first command requires an existing `macos/.venv`; omit
`OGMA_SKIP_BOOTSTRAP=1` to allow dependency bootstrap. The Python suite needs
only the standard library, plus Xcode Command Line Tools for its macOS native
tests. Its temporary Unix sockets require an environment that permits local
socket binding. Neither command needs provider credentials.

See [the macOS review](REVIEW.md) for findings, remaining work, and validation limits.
