...
speak.sh uses a PID file (`ogma_tts.pid`) for toggle behavior:
...
- speak.sh creates temp files/dirs with prefix `ogma_tts_`.
...
- The daemon cleans up orphaned `ogma_tts_*` dirs on startup.
...
All three sources (speak.sh, Ogma.swift, install.command) must agree on defaults.
...
| Path | Purpose |
|---|---|
| `~/.config/ogma/config` | Config file (shared between Swift and bash) |
...
| `~/.local/bin/ogma-audio` | Compiled CoreAudio CLI (mute check/unmute) |
| `~/.local/bin/uninstall.command` | Installed uninstaller |
| `~/.local/share/ogma/venv/` | Python venv with mlx-audio |
| `~/.local/share/ogma/tts.sock` | Daemon Unix socket |
| `~/.local/share/ogma/tts_server.pid` | Daemon PID file |
| `~/.local/share/ogma/tts_server.lock` | Daemon flock file |
| `~/.local/share/ogma/tts.log` | Shared log file |
| `~/.local/share/ogma/install.log` | Installer error log (pip, swiftc output) |
| `$TMPDIR/ogma_tts.pid` | speak.sh PID file |
| `$TMPDIR/ogma_text` | Full text for respeak |
| `$TMPDIR/ogma_status` | Playback position for respeak |
| `~/Applications/Ogma.app` | Compiled menu bar app |
| `~/Library/Services/Speak Selection.workflow` | Automator Quick Action |
| `/tmp/ogma_install.lock` | Installer single-instance lock |
...
16. **Installer is single-instance.** `mkdir /tmp/ogma_install.lock` acts as
    an atomic lock. Stale locks (dead PID) are reclaimed. The lock is removed in
    the EXIT trap.

17. **Re-install preserves user config.** When `~/.config/ogma/config` already
...
22. **Mute check is three-tier.** From the app: in-process CoreAudio
    (microseconds). Standalone: `ogma-audio` CLI (35ms). Last resort:
    osascript (80-500ms). `OGMA_MUTE_CHECKED=1` env var tells speak.sh
...
24. **Releases decouple users from HEAD.** Download links point to
    `releases/latest/download/ogma.zip`, not the main branch archive.
    To cut a new release:
    `git archive --format=zip --prefix=ogma/ -o /tmp/ogma.zip HEAD`
    then `gh release create vX.Y.Z /tmp/ogma.zip --title "Ogma vX.Y.Z" --notes "..."`.
...
25. **Text normalization runs before TTS.** `normalize_text()` in speak.sh
    ...
    **LaTeX front-end (`_frontend_latex`):** L1: comment/preamble stripping.
    L2: custom macro expansion (with ~/.config/ogma/latex_macros.tex cache).
...
26. **API key is validated on entry.** Both `install.command` and
    `OgmaSettings.swift` validate the API key by calling
