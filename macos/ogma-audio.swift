// ogma-audio — CoreAudio utilities for Ogma
// Usage: ogma-audio is-muted     → exit 0 if muted, 1 if not
//        ogma-audio unmute       → exit 0 on success, 1 on failure
//        ogma-audio play-queue   → gapless audio queue player (stdin/stdout)
...
guard CommandLine.arguments.count >= 2 else {
    fputs("Usage: ogma-audio <is-muted|unmute|play-queue>\n", stderr)
    exit(2)
}
