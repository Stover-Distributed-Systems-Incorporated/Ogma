...
SETTINGS_SWIFT="$SCRIPT_DIR/Ogma.swift"
FAST=false
FILTER=""
...
export TTS_SOCK="/tmp/ogma_test_nosock_$$"
...
check_exit "auto + no API key + no local TTS → exits 1" 1 \
    bash -c 'echo "hello world" | env PATH="'"$_STUBS"':$PATH" VENV_PYTHON="'"$_STUBS"'/python3" ELEVENLABS_API_KEY="" TTS_BACKEND=auto OGMA_NO_QUEUE_PLAYER=1 bash "'"$SPEAK_SH"'"'
...
# ── 12. PID file uses ogma_tts prefix ─────────────────────────

section "PID file prefix"

check "speak.sh uses ogma_tts.pid" \
    "yes" "$(grep -q 'ogma_tts\.pid' "$SPEAK_SH" && echo "yes" || echo "no")"
check "speak.sh does not use elevenlabs_tts.pid" \
    "yes" "$(! grep -q 'elevenlabs_tts\.pid' "$SPEAK_SH" && echo "yes" || echo "no")"
check "speak.sh uses ogma_tts_ temp file prefix" \
    "yes" "$(grep -q 'ogma_tts_' "$SPEAK_SH" && echo "yes" || echo "no")"
...
check_exit "TTS_BACKEND=local with no API key → exits 0 (key not needed)" 0 \
    bash -c 'echo "hello" | env PATH="'"$_STUBS"':$PATH" VENV_PYTHON="'"$_STUBS"'/python3" TTS_BACKEND=local LOCAL_VOICE=af_heart OGMA_NO_QUEUE_PLAYER=1 bash "'"$SPEAK_SH"'"'
...
bash -c 'echo "hello" | env PATH="'"$_STUBS"':$PATH" VENV_PYTHON="'"$_STUBS"'/python3" TTS_BACKEND=local OGMA_NO_QUEUE_PLAYER=1 bash "'"$SPEAK_SH"'"' >/dev/null 2>&1 || true
...
rm -f "$_MARKERS/curl_called" "$_MARKERS/mlx_called"
bash -c 'echo "hello" | env PATH="'"$_STUBS"':$PATH" VENV_PYTHON="'"$_STUBS"'/python3" TTS_BACKEND=auto OGMA_NO_QUEUE_PLAYER=1 bash "'"$SPEAK_SH"'"' >/dev/null 2>&1 || true
...
bash -c 'echo "hello" | env PATH="'"$_STUBS"':$PATH" VENV_PYTHON="'"$_STUBS"'/python3" TTS_BACKEND=elevenlabs TTS_BACKENDS_INSTALLED=elevenlabs OGMA_NO_QUEUE_PLAYER=1 bash "'"$SPEAK_SH"'"' >/dev/null 2>&1 || true
...
check_exit "local TTS generation failure → exits 1" 1 \
    bash -c 'echo "hello" | env PATH="'"$_STUBS"':$PATH" VENV_PYTHON="'"$_STUBS"'/python3" TTS_BACKEND=local OGMA_NO_QUEUE_PLAYER=1 bash "'"$SPEAK_SH"'"'
...
check_exit "429 + both installed → exits 0 (silent fallback)" 0 \
    bash -c 'echo "hello" | env PATH="'"$_STUBS"':$PATH" VENV_PYTHON="'"$_STUBS"'/python3" TTS_BACKEND=auto TTS_BACKENDS_INSTALLED=both OGMA_MUTE_CHECKED=1 OGMA_NO_QUEUE_PLAYER=1 bash "'"$SPEAK_SH"'"'
...
check_exit "network failure + elevenlabs only → exits 1" 1 \
    bash -c 'echo "hello" | env PATH="'"$_STUBS"':$PATH" VENV_PYTHON="'"$_STUBS"'/python3" TTS_BACKEND=auto TTS_BACKENDS_INSTALLED=elevenlabs OGMA_NO_QUEUE_PLAYER=1 bash "'"$SPEAK_SH"'"'
...
check "speak.sh defines TEXT_FILE path" \
    "yes" "$(grep -q 'TEXT_FILE=.*ogma_text' "$SPEAK_SH" && echo "yes" || echo "no")"
check "speak.sh defines STATUS_FILE path" \
    "yes" "$(grep -q 'STATUS_FILE=.*ogma_status' "$SPEAK_SH" && echo "yes" || echo "no")"
...
echo "Hello world. This is a test sentence." | \
    env PATH="$_STUBS:$PATH" VENV_PYTHON="$_STUBS/python3" TMPDIR="$_TESTTMP" TTS_BACKEND=auto OGMA_NO_QUEUE_PLAYER=1 \
    bash "$SPEAK_SH" >/dev/null 2>&1 || true

check "TEXT_FILE created with piped text" \
    "Hello world. This is a test sentence." "$(cat "$_TESTTMP/ogma_text" 2>/dev/null)"

check "STATUS_FILE created" \
    "yes" "$([ -f "$_TESTTMP/ogma_status" ] && echo "yes" || echo "no")"

check "STATUS_FILE has four lines (epoch, duration, offset, len)" \
    "4" "$([ -f "$_TESTTMP/ogma_status" ] && wc -l < "$_TESTTMP/ogma_status" | tr -d ' ' || echo "0")"
...
_STATUS_EPOCH=$(head -1 "$_TESTTMP/ogma_status" 2>/dev/null || echo "0")
...
check "STATUS_FILE contains audio duration" \
    "5.000000" "$([ -f "$_TESTTMP/ogma_status" ] && sed -n '2p' "$_TESTTMP/ogma_status" || echo "none")"
...
echo "Local TTS test sentence." | \
    env PATH="$_STUBS:$PATH" VENV_PYTHON="$_STUBS/python3" TMPDIR="$_TESTTMP" TTS_BACKEND=local LOCAL_VOICE=af_heart OGMA_NO_QUEUE_PLAYER=1 \
    bash "$SPEAK_SH" >/dev/null 2>&1 || true

check "local backend: TEXT_FILE created" \
    "Local TTS test sentence." "$(cat "$_TESTTMP/ogma_text" 2>/dev/null)"
...
bash -c 'echo "hello" | env PATH="'"$_STUBS"':$PATH" VENV_PYTHON="'"$_STUBS"'/python3" TTS_BACKEND=auto ELEVENLABS_API_KEY="" OGMA_NO_QUEUE_PLAYER=1 bash "'"$SPEAK_SH"'"' >/dev/null 2>&1 || true
...
check_exit "auto + no API key → exits 0 (silent local)" 0 \
    bash -c 'echo "hello" | env PATH="'"$_STUBS"':$PATH" VENV_PYTHON="'"$_STUBS"'/python3" TTS_BACKEND=auto ELEVENLABS_API_KEY="" OGMA_NO_QUEUE_PLAYER=1 bash "'"$SPEAK_SH"'"'
...
check_exit "auto + network failure → exits 0 (falls back)" 0 \
    bash -c 'echo "hello" | env PATH="'"$_STUBS"':$PATH" VENV_PYTHON="'"$_STUBS"'/python3" TTS_BACKEND=auto OGMA_MUTE_CHECKED=1 OGMA_NO_QUEUE_PLAYER=1 bash "'"$SPEAK_SH"'"'
...
echo "test" | env PATH="$_STUBS:$PATH" VENV_PYTHON="$_STUBS/python3" TMPDIR="$_TESTTMP" TTS_BACKEND=local LOCAL_VOICE=af_heart OGMA_NO_QUEUE_PLAYER=1 \
    bash "$SPEAK_SH" >/dev/null 2>&1 || true
...
echo "test" | env PATH="$_STUBS:$PATH" VENV_PYTHON="$_STUBS/python3" TMPDIR="$_TESTTMP" TTS_BACKEND=local LOCAL_VOICE=bf_emma OGMA_NO_QUEUE_PLAYER=1 \
    bash "$SPEAK_SH" >/dev/null 2>&1 || true
...
check "uses xcrun swiftc for compilation" \
    "yes" "$(grep -q 'xcrun swiftc.*Ogma.swift' "$SCRIPT_DIR/install.command" && echo "yes" || echo "no")"
...
check "§4: single-instance lock directory created" \
    "yes" "$(grep -q 'mkdir.*\$_LOCKDIR\|mkdir.*ogma_install' "$SCRIPT_DIR/install.command" && echo "yes" || echo "no")"

check "§4: lock directory cleaned up in cleanup/trap" \
    "yes" "$(grep -q 'rm.*\$_LOCKDIR\|rm.*ogma_install' "$SCRIPT_DIR/install.command" && echo "yes" || echo "no")"
...
check "§16: login item checked before adding" \
    "yes" "$(grep -q 'login item.*Ogma\|every login item' "$SCRIPT_DIR/install.command" \
...
_LOCKDIR=$(mktemp -d)/ogma_install.lock
...
# misaki==0.8.4 requires Python >=3.8,<3.13; find_python must reject 3.13+
# so a 3.13 interpreter isn't chosen for the venv (would fail to install).
check "install-local.sh: rejects Python 3.13+ (misaki==0.8.4 upper bound)" \
    "yes" "$(grep -q '< *(3,13)' "$INSTALL_LOCAL" && echo "yes" || echo "no")"
...
if [ "${1:-}" = "--fast" ] || [ ! -x "$VENV_PY" ]; then
    ...
    _TTS_TMPD=$(mktemp -d /tmp/ogma_test_tts_XXXXXXXXXX)
    _tts_ok="no"
    if (cd "$_TTS_TMPD" && "$VENV_PY" -m mlx_audio.tts.generate \
            ...
            --file_prefix ogma \
            --audio_format wav \
            --join_audio >/dev/null 2>&1) && [ -s "$_TTS_TMPD/ogma.wav" ]; then
        _tts_ok="yes"
    fi
    rm -rf "$_TTS_TMPD"
    check "local TTS generates ogma.wav" "yes" "$_tts_ok"
...
check "uninstall.command: kills TTS daemon" \
    "yes" "$(grep -q '\${_svc}.pid' "$SCRIPT_DIR/uninstall.command" && echo "yes" || echo "no")"
...
# The model must unload after idle in every mode (frees ~350 MB). The idle
# watchdog is started unconditionally; managed mode ALSO adds parent_watchdog.
check "tts_server.py: default idle timeout is 120s (~2 min)" \
    "yes" "$(grep -q 'DEFAULT_IDLE_TIMEOUT = 120' "$TTS_SERVER" && echo "yes" || echo "no")"

check "tts_server.py: idle watchdog runs in managed mode too (unconditional)" \
    "yes" "$(grep -q 'threading.Thread(target=idle_watchdog, daemon=True).start()' "$TTS_SERVER" && echo "yes" || echo "no")"

# Menu-driven auto-unload: timeout is resolved fresh (config wins → live) and
# state is published for the countdown; state file cleaned up on shutdown.
check "tts_server.py: idle timeout resolved live via effective_timeout()" \
    "yes" "$(grep -q 'def effective_timeout' "$TTS_SERVER" && \
             grep -q 'LOCAL_IDLE_TIMEOUT' "$TTS_SERVER" && echo "yes" || echo "no")"

check "tts_server.py: publishes state file for menu countdown" \
    "yes" "$(grep -q 'def write_state' "$TTS_SERVER" && \
             grep -q 'tts_state.json' "$TTS_SERVER" && echo "yes" || echo "no")"

check "tts_server.py: removes state file on shutdown" \
    "yes" "$(grep -q 'SOCKET_PATH, PID_FILE, STATE_FILE' "$TTS_SERVER" && echo "yes" || echo "no")"

# Work around the mlx-audio iSTFTNet broadcast bug that made non-default
# speeds fail ("Shapes (1,N,1) and (1,N+300,9) cannot be broadcast").
check "tts_server.py: patches Kokoro _f02sine speed/length bug" \
    "yes" "$(grep -q 'def patch_kokoro_speed_bug' "$TTS_SERVER" && \
             grep -q '_f02sine' "$TTS_SERVER" && echo "yes" || echo "no")"

check "tts_server.py: applies the speed patch after model load" \
    "yes" "$(awk '/def load_tts_model/,/model loaded/' "$TTS_SERVER" | grep -q 'patch_kokoro_speed_bug()' && echo "yes" || echo "no")"

# ── STT (dictation) daemon + integration ─────────────────────────
STT_SERVER="$SCRIPT_DIR/stt_server.py"

check "stt_server.py exists" \
    "yes" "$([ -f "$STT_SERVER" ] && echo "yes" || echo "no")"

check "stt_server.py: valid Python syntax" \
    "0" "$(python3 -c "import ast,sys; ast.parse(open('$STT_SERVER').read())" 2>/dev/null; echo $?)"

check "stt_server.py: own Unix socket (stt.sock)" \
    "yes" "$(grep -q 'stt.sock' "$STT_SERVER" && grep -q 'AF_UNIX' "$STT_SERVER" && echo "yes" || echo "no")"

check "stt_server.py: loads Parakeet via parakeet_mlx" \
    "yes" "$(grep -q 'from parakeet_mlx import from_pretrained' "$STT_SERVER" && echo "yes" || echo "no")"

check "stt_server.py: transcribes on the main thread (MLX streams thread-local)" \
    "yes" "$(grep -q 'handle_client(conn)' "$STT_SERVER" && ! grep -q 'target=handle_client' "$STT_SERVER" && echo "yes" || echo "no")"

check "stt_server.py: idle-unload + state file for menu" \
    "yes" "$(grep -q 'def idle_watchdog' "$STT_SERVER" && grep -q 'stt_state.json' "$STT_SERVER" && echo "yes" || echo "no")"

# A GUI-launched daemon has a restricted PATH without ffmpeg; decode with
# soundfile+get_logmel instead of model.transcribe() (which shells to ffmpeg).
check "stt_server.py: decodes audio without ffmpeg (soundfile + get_logmel)" \
    "yes" "$(grep -q 'import soundfile' "$STT_SERVER" && \
             grep -q 'get_logmel' "$STT_SERVER" && \
             grep -q 'model.generate(mel)' "$STT_SERVER" && echo "yes" || echo "no")"

# Live streaming: header {mode:stream} → frames → {partial} lines → {final}.
check "stt_server.py: streaming mode via transcribe_stream" \
    "yes" "$(grep -q 'def handle_stream' "$STT_SERVER" && \
...
check "Ogma.swift: live streaming client + floating caption overlay" \
    "yes" "$(grep -q 'class STTStreamClient' "$SETTINGS_SWIFT" && \
             ...
             grep -q 'AVAudioEngine' "$SETTINGS_SWIFT" && echo "yes" || echo "no")"

check "Ogma.swift: caption panel is non-activating (never steals focus)" \
    "yes" "$(grep -q 'nonactivatingPanel' "$SETTINGS_SWIFT" && \
             grep -q 'ignoresMouseEvents' "$SETTINGS_SWIFT" && echo "yes" || echo "no")"

check "Ogma.swift: card survives app switches (no hide-on-deactivate, all Spaces)" \
    "yes" "$(grep -q 'hidesOnDeactivate = false' "$SETTINGS_SWIFT" && \
             grep -q 'canJoinAllSpaces' "$SETTINGS_SWIFT" && echo "yes" || echo "no")"

check "install-local.sh: installs parakeet-mlx" \
    "yes" "$(grep -q 'parakeet-mlx' "$SCRIPT_DIR/install-local.sh" && echo "yes" || echo "no")"

check "install.command: copies stt_server.py" \
    "yes" "$(grep -q 'cp -f.*stt_server.py' "$SCRIPT_DIR/install.command" && echo "yes" || echo "no")"

check "install.command: declares NSMicrophoneUsageDescription" \
    "yes" "$(grep -q 'NSMicrophoneUsageDescription' "$SCRIPT_DIR/install.command" && echo "yes" || echo "no")"

check "Ogma.swift: push-to-talk dictation hotkey + injection" \
    "yes" "$(grep -q 'handleDictateHotkey' "$SETTINGS_SWIFT" && \
             grep -q 'AVAudioEngine' "$SETTINGS_SWIFT" && \
             grep -q 'func injectText' "$SETTINGS_SWIFT" && echo "yes" || echo "no")"
...
check "Ogma.swift: startTTSDaemon method" \
    "yes" "$(grep -q 'startTTSDaemon' "$SETTINGS_SWIFT" && echo "yes" || echo "no")"

check "Ogma.swift: stopTTSDaemon method" \
    "yes" "$(grep -q 'stopTTSDaemon' "$SETTINGS_SWIFT" && echo "yes" || echo "no")"

check "Ogma.swift: applicationWillTerminate stops daemon" \
    "yes" "$(grep -q 'applicationWillTerminate' "$SETTINGS_SWIFT" && echo "yes" || echo "no")"

check "Ogma.swift: applicationWillTerminate kills current process" \
    "yes" "$(awk '/applicationWillTerminate/,/^    }/' "$SETTINGS_SWIFT" | grep -q 'killCurrentProcess' && echo "yes" || echo "no")"

# ── STT word confidence + dictation review card ──────────────────
section "STT word confidence + review card"

check "stt_server.py: words_from_result exists" \
    "yes" "$(grep -q 'def words_from_result' "$STT_SERVER" && echo "yes" || echo "no")"

check "stt_server.py: partials, finals, and one-shot carry words" \
    "yes" "$(grep -q '"words": add_suggestions(' "$STT_SERVER" && \
             grep -q '"words": final_words' "$STT_SERVER" && \
             grep -q '"partial": text, "words": words' "$STT_SERVER" && echo "yes" || echo "no")"

check "stt_server.py: OGMA_DATA_DIR isolates data dir AND config" \
    "yes" "$(grep -q 'OGMA_DATA_DIR' "$STT_SERVER" && \
             grep -q '_data_dir_override' "$STT_SERVER" && echo "yes" || echo "no")"

# Word merge invariants: " ".join(words) must reproduce result.text exactly
# (including the "!"/"?" sentence-split and doubled-space token cases), and
# a word's confidence is the min of its tokens'.
_wfr_ok=$(python3 - "$STT_SERVER" <<'PYEOF'
import ast, sys
tree = ast.parse(open(sys.argv[1]).read())
fn = next(n for n in ast.walk(tree)
          if isinstance(n, ast.FunctionDef) and n.name == "words_from_result")
ns = {}
exec(compile(ast.Module(body=[fn], type_ignores=[]), "wfr", "exec"), ns)
wfr = ns["words_from_result"]
class T:
    def __init__(s, text, conf): s.text, s.confidence = text, conf
class R:
    def __init__(s, text, toks): s.text, s.tokens = text, toks
cases = [
    (" Hello world.", [T(" Hello", .95), T(" wor", .6), T("ld", .9), T(".", .99)]),
    (" Yahoo!'s stock", [T(" Yahoo", .9), T("!", .8), T("'", .7), T("s", .95), T(" stock", .9)]),
    (" What?!", [T(" What", .9), T("?", .8), T("!", .7)]),
    (" Hello  world", [T(" Hello", .9), T(" ", .5), T(" world", .8)]),
]
for text, toks in cases:
    out = wfr(R(text, toks))
    assert " ".join(w["text"] for w in out) == text.strip(), (text, out)
    assert all(0.0 <= w["confidence"] <= 1.0 for w in out), out
out = wfr(R(" Hello world.", cases[0][1]))
assert out[0]["confidence"] == 0.95 and out[1]["confidence"] == 0.6, out
assert wfr(R("", [])) == []
print("ok")
PYEOF
) || true
check "words_from_result: join reproduces transcript, min-confidence per word" \
    "ok" "$_wfr_ok"

# Filler filtering: disfluencies dropped, capitalization/punctuation repaired,
# real words containing filler substrings kept.
_ff_ok=$(python3 - "$STT_SERVER" 2>/dev/null <<'PYEOF'
import ast, sys
tree = ast.parse(open(sys.argv[1]).read())
ns = {}
for n in ast.walk(tree):
    if isinstance(n, ast.FunctionDef) and n.name == "strip_fillers":
        fn = n
    if isinstance(n, ast.Assign) and getattr(n.targets[0], "id", "") in ("FILLER_WORDS", "_PUNCT"):
        exec(compile(ast.Module(body=[n], type_ignores=[]), "s", "exec"), ns)
exec(compile(ast.Module(body=[fn], type_ignores=[]), "s", "exec"), ns)
sf = ns["strip_fillers"]
W = lambda t: {"text": t, "confidence": 0.9}
assert [w["text"] for w in sf([W("Um,"), W("hello"), W("there.")])] == ["Hello", "there."]
assert [w["text"] for w in sf([W("I"), W("think"), W("um.")])] == ["I", "think."]
assert [w["text"] for w in sf([W("so,"), W("um,"), W("yeah.")])] == ["so,", "yeah."]
assert len(sf([W("Ergonomics"), W("hummingbird"), W("Umbrella")])) == 3
print("ok")
PYEOF
) || true
check "filler words stripped (um/uh), sentence shape preserved" "ok" "$_ff_ok"

# Personal dictionary: phonetic rescue of user words (key-distance ranked),
# protection + casing alignment for the user's own words, live reload.
if "$VENV_PYTHON" -c "import jellyfish" 2>/dev/null; then
    _dict_ok=$("$VENV_PYTHON" - "$STT_SERVER" 2>/dev/null <<'PYEOF'
import importlib.util, os, sys, tempfile
d = tempfile.mkdtemp(prefix="ogdict")
os.makedirs(os.path.join(d, "ngrams"))   # no ngram data: dictionary works alone
open(os.path.join(d, "dictionary.txt"), "w").write("Xanthippe\nKubecon\n")
os.environ["OGMA_DATA_DIR"] = d
spec = importlib.util.spec_from_file_location("stt_server", sys.argv[1])
m = importlib.util.module_from_spec(spec)
spec.loader.exec_module(m)
w = [{"text": "Zantipi,", "confidence": 0.9}]
m.add_suggestions(w)
assert w[0]["suggestion"] == "Xanthippe," and w[0]["confidence"] == 0.87, w
w = [{"text": "kubecon.", "confidence": 0.5}]
m.add_suggestions(w)
assert w[0]["suggestion"] == "Kubecon.", w
w = [{"text": "meeting", "confidence": 0.5}]
m.add_suggestions(w)
assert "suggestion" not in w[0], w
print("ok")
PYEOF
    ) || _dict_ok="no"
    check "personal dictionary: phonetic rescue + own-word protection" \
        "ok" "$_dict_ok"
else
    printf "  %s  %s\n" "SKIP" "dictionary test: jellyfish not in dev venv"
fi

# Context autocorrect: a low-confidence homophone gets a suggestion from
# bigram context; confident words are left alone. Fixture ngram data via
# OGMA_DATA_DIR; needs jellyfish in the dev venv.
if "$VENV_PYTHON" -c "import jellyfish" 2>/dev/null; then
    _ac_ok=$("$VENV_PYTHON" - "$STT_SERVER" 2>/dev/null <<'PYEOF'
import importlib.util, os, sys, tempfile
d = tempfile.mkdtemp(prefix="s11ng")
ng = os.path.join(d, "ngrams"); os.makedirs(ng)
open(os.path.join(ng, "count_1w.txt"), "w").write("is\t900000\nthere\t500000\ntheir\t400000\n")
open(os.path.join(ng, "count_2w.txt"), "w").write("there is\t50000\ntheir is\t2\n")
os.environ["OGMA_DATA_DIR"] = d
spec = importlib.util.spec_from_file_location("stt_server", sys.argv[1])
m = importlib.util.module_from_spec(spec)
spec.loader.exec_module(m)
words = [{"text": "Their", "confidence": 0.4}, {"text": "is", "confidence": 1.0}]
m.add_suggestions(words)
assert words[0].get("suggestion") == "There", words
assert "suggestion" not in words[1], words
confident = [{"text": "Their", "confidence": 0.95}]
m.add_suggestions(confident)
assert "suggestion" not in confident[0], confident
print("ok")
PYEOF
    ) || _ac_ok="no"
    check "autocorrect: context suggestion for low-confidence homophone" \
        "ok" "$_ac_ok"
else
    printf "  %s  %s\n" "SKIP" "autocorrect test: jellyfish not in dev venv"
fi

check "install-local.sh: wires jellyfish + ngram data" \
    "yes" "$(grep -q 'jellyfish' "$SCRIPT_DIR/install-local.sh" && \
             grep -q 'count_1w.txt' "$SCRIPT_DIR/install-local.sh" && echo "yes" || echo "no")"

check "Ogma.swift: parses word confidences from the daemon" \
    "yes" "$(grep -q 'struct STTWord' "$SETTINGS_SWIFT" && \
             grep -q '\"words\"' "$SETTINGS_SWIFT" && echo "yes" || echo "no")"

check "Ogma.swift: review card (nonactivating key panel, insert/edit/discard)" \
    "yes" "$(grep -q 'class ReviewPanel' "$SETTINGS_SWIFT" && \
             grep -q 'canBecomeKey' "$SETTINGS_SWIFT" && \
             grep -q 'func showReview' "$SETTINGS_SWIFT" && \
             grep -q 'DICTATION_REVIEW' "$SETTINGS_SWIFT" && echo "yes" || echo "no")"
...
check "Ogma.swift: dictionary editor in the menu" \
    "yes" "$(grep -q 'func editDictionary' "$SETTINGS_SWIFT" && \
             grep -q 'dictionary.txt' "$SETTINGS_SWIFT" && echo "yes" || echo "no")"

check "Ogma.swift: hotkey tap auto-recovery (health check + reinstall)" \
    "yes" "$(grep -q 'checkHotkeyHealth' "$SETTINGS_SWIFT" && \
             grep -q 'tapIsEnabled' "$SETTINGS_SWIFT" && \
             grep -q 'func removeHotkey' "$SETTINGS_SWIFT" && echo "yes" || echo "no")"

# Live end-to-end: isolated daemon (OGMA_DATA_DIR), say-generated speech
# streamed in app-sized chunks; final must carry words whose join equals the
# transcript. Skipped in --fast mode or without parakeet-mlx.
if $_SKIP_SECTION; then
    :   # section filtered out — this raw block doesn't go through check()
elif $FAST || [ ! -x "$VENV_PYTHON" ] || ! "$VENV_PYTHON" -c "import parakeet_mlx" 2>/dev/null; then
    printf "  %s  %s\n" "SKIP" "live STT stream: --fast or parakeet-mlx not installed"
else
    _STT_DIR=$(mktemp -d /tmp/s11sttest.XXXXXX)   # short path: AF_UNIX limit
    { say -o "$_STT_DIR/t.aiff" "testing one two three" 2>/dev/null && \
        afconvert -f WAVE -d LEI16@16000 -c 1 "$_STT_DIR/t.aiff" "$_STT_DIR/t.wav" 2>/dev/null ; } || true
    _stt_ok="no"
    if [ -s "$_STT_DIR/t.wav" ]; then
        OGMA_DATA_DIR="$_STT_DIR" "$VENV_PYTHON" "$STT_SERVER" >/dev/null 2>&1 &
        _STT_PID=$!
        for _i in $(seq 1 120); do [ -S "$_STT_DIR/stt.sock" ] && break; sleep 0.5; done
        if [ -S "$_STT_DIR/stt.sock" ]; then
            _stt_ok=$("$VENV_PYTHON" - "$_STT_DIR/stt.sock" "$_STT_DIR/t.wav" 2>/dev/null <<'PYEOF'
import json, socket, struct, sys
import soundfile as sf
samples, sr = sf.read(sys.argv[2], dtype="float32", always_2d=False)
s = socket.socket(socket.AF_UNIX, socket.SOCK_STREAM)
s.settimeout(60)
s.connect(sys.argv[1])
s.sendall(b'{"mode":"stream","sample_rate":16000}\n')
chunk = int(0.085 * sr)
for i in range(0, len(samples), chunk):
    p = samples[i:i + chunk].astype("<f4").tobytes()
...
while final is None:
    ...
    while b"\n" in buf:
        ...
        if "final" in obj:
            final = obj
assert final and final["final"].strip(), final
words = final["words"]
assert words and " ".join(w["text"] for w in words) == final["final"], final
assert all(0.0 <= w["confidence"] <= 1.0 for w in words), words
print("ok")
PYEOF
            ) || _stt_ok="no"
        fi
        kill $_STT_PID 2>/dev/null || true
        wait $_STT_PID 2>/dev/null || true
    fi
    rm -rf "$_STT_DIR"
    check "live stream: final carries words, join==final, confidences valid" \
        "ok" "$_stt_ok"
fi
...
check "Ogma.swift: localSpeed config field" \
    "yes" "$(grep -q 'localSpeed' "$SETTINGS_SWIFT" && echo "yes" || echo "no")"

check "Ogma.swift: pickLocalSpeed handler" \
    "yes" "$(grep -q 'pickLocalSpeed' "$SETTINGS_SWIFT" && echo "yes" || echo "no")"

check "Ogma.swift: LOCAL_SPEED in config save" \
    "yes" "$(grep -q 'LOCAL_SPEED' "$SETTINGS_SWIFT" && echo "yes" || echo "no")"

check "Ogma.swift: default local voice is bf_lily" \
...
check "tts_server.py: cleans up ogma_tts_ temp dirs on startup" \
    "yes" "$(grep -q 'ogma_tts_' "$TTS_SERVER" && grep -q 'shutil.rmtree' "$TTS_SERVER" && echo "yes" || echo "no")"
...
_TESTTMP=$(mktemp -d "${TMPDIR:-/tmp}/ogma_test_XXXXXXXXXX")
...
(
    ...
    _PID_FILE="$_TESTTMP/ogma_tts.pid"
...
(
    _PID_FILE="$_TESTTMP/ogma_tts2.pid"
...
(
    set +e  # match speak.sh behavior (no set -e)
    _PID_FILE="$_TESTTMP/ogma_tts3.pid"
...
_SIMDIR=$(mktemp -d "${TMPDIR:-/tmp}/ogma_sim_XXXXXXXXXX")
...
run_local_tts() {
    ...
    TMP_DIR=$(mktemp -d "${TMPDIR:-/tmp}/ogma_sim_gen_XXXXXXXXXX")
    TMP_FILE="$TMP_DIR/ogma.wav"
...
run_elevenlabs_tts() {
    local sentence="$1"
    TMP_FILE=$(mktemp "${TMPDIR:-/tmp}/ogma_sim_tts_XXXXXXXXXX")
...
_STATUS_OFFSET=$(sed -n '3p' "$_TESTTMP/ogma_status" 2>/dev/null)
_STATUS_LEN=$(sed -n '4p' "$_TESTTMP/ogma_status" 2>/dev/null)
...
printf '%s' "$_BIGTEXT" | env PATH="$_STUBS:$PATH" VENV_PYTHON="$_STUBS/python3" \
    TMPDIR="$_TESTTMP" TTS_BACKEND=local LOCAL_VOICE=af_heart OGMA_NO_QUEUE_PLAYER=1 \
...
_WAV_TMP=$(mktemp "${TMPDIR:-/tmp/}ogma_test_XXXXXXXXXX.wav")
...
_AUDIO_SWIFT="$SCRIPT_DIR/ogma-audio.swift"

check "ogma-audio.swift exists" \
    "yes" "$([ -f "$_AUDIO_SWIFT" ] && echo "yes" || echo "no")"

check "ogma-audio.swift imports CoreAudio" \
    "yes" "$(grep -q 'import CoreAudio' "$_AUDIO_SWIFT" 2>/dev/null && echo "yes" || echo "no")"

check "ogma-audio.swift uses kAudioDevicePropertyMute" \
    "yes" "$(grep -q 'kAudioDevicePropertyMute' "$_AUDIO_SWIFT" 2>/dev/null && echo "yes" || echo "no")"

check "ogma-audio.swift handles is-muted subcommand" \
    "yes" "$(grep -q 'is-muted' "$_AUDIO_SWIFT" 2>/dev/null && echo "yes" || echo "no")"

check "ogma-audio.swift handles unmute subcommand" \
    "yes" "$(grep -q '"unmute"' "$_AUDIO_SWIFT" 2>/dev/null && echo "yes" || echo "no")"

check "speak.sh: mute check uses ogma-audio" \
    "yes" "$(grep -q 'ogma-audio' "$SPEAK_SH" && grep -q '_AUDIO_TOOL.*is-muted' "$SPEAK_SH" && echo "yes" || echo "no")"

check "speak.sh: unmute uses ogma-audio" \
    "yes" "$(grep -q 'ogma-audio' "$SPEAK_SH" && grep -q '_AUDIO_TOOL.*unmute' "$SPEAK_SH" && echo "yes" || echo "no")"

check "speak.sh: osascript mute fallback for missing binary" \
    "yes" "$(grep -q 'output muted of (get volume settings)' "$SPEAK_SH" && echo "yes" || echo "no")"

check "install.command: compiles ogma-audio.swift" \
    "yes" "$(grep -q 'ogma-audio.swift' "$SCRIPT_DIR/install.command" && echo "yes" || echo "no")"

# In-process mute check in Ogma.swift (no fork when launched from app)
check "Ogma.swift: imports CoreAudio" \
    "yes" "$(grep -q 'import CoreAudio' "$SCRIPT_DIR/Ogma.swift" && echo "yes" || echo "no")"

check "Ogma.swift: has isOutputMuted function" \
    "yes" "$(grep -q 'func isOutputMuted' "$SCRIPT_DIR/Ogma.swift" && echo "yes" || echo "no")"

check "Ogma.swift: has unmuteOutput function" \
    "yes" "$(grep -q 'func unmuteOutput' "$SCRIPT_DIR/Ogma.swift" && echo "yes" || echo "no")"

check "Ogma.swift: checks mute in runSpeak" \
    "yes" "$(grep -q 'isOutputMuted' "$SCRIPT_DIR/Ogma.swift" && echo "yes" || echo "no")"

check "Ogma.swift: passes OGMA_MUTE_CHECKED to speak.sh" \
    "yes" "$(grep -q 'OGMA_MUTE_CHECKED' "$SCRIPT_DIR/Ogma.swift" && echo "yes" || echo "no")"

# Lazy local-model loading + menu "model loaded" indicator.
check "Ogma.swift: loads local model lazily in runSpeak (not at launch)" \
    "yes" "$(awk '/func runSpeak/,/isOutputMuted/' "$SCRIPT_DIR/Ogma.swift" | grep -q 'startTTSDaemon()' && echo "yes" || echo "no")"

# Interactive model control: clickable load/unload toggle, live countdown,
# and an auto-unload timeout picker persisted to config.
check "Ogma.swift: model line is a clickable load/unload toggle" \
    "yes" "$(grep -q 'func toggleModel' "$SCRIPT_DIR/Ogma.swift" && \
             grep -q 'func unloadModel' "$SCRIPT_DIR/Ogma.swift" && echo "yes" || echo "no")"

check "Ogma.swift: shows live 'Unloading in' countdown" \
    "yes" "$(grep -q 'Unloading in' "$SCRIPT_DIR/Ogma.swift" && \
             grep -q 'func remainingSeconds' "$SCRIPT_DIR/Ogma.swift" && echo "yes" || echo "no")"

check "Ogma.swift: countdown timer runs during menu tracking (.common mode)" \
    "yes" "$(grep -q 'forMode: .common' "$SCRIPT_DIR/Ogma.swift" && \
             grep -q 'func menuDidClose' "$SCRIPT_DIR/Ogma.swift" && echo "yes" || echo "no")"

check "Ogma.swift: auto-unload timeout picker persists to config" \
    "yes" "$(grep -q 'func pickIdleTimeout' "$SCRIPT_DIR/Ogma.swift" && \
             grep -q 'LOCAL_IDLE_TIMEOUT' "$SCRIPT_DIR/Ogma.swift" && echo "yes" || echo "no")"

check "speak.sh: skips mute check when OGMA_MUTE_CHECKED=1" \
    "yes" "$(grep -q 'OGMA_MUTE_CHECKED' "$SPEAK_SH" && echo "yes" || echo "no")"

# Cmd+V paste support: dialogs with text fields must use .regular activation policy
check "Ogma.swift: API key dialog enables paste (regular activation)" \
    "yes" "$(awk '/func showAPIKeyDialog/,/^    }/' "$SCRIPT_DIR/Ogma.swift" | grep -q 'setActivationPolicy(.regular)' && echo "yes" || echo "no")"

check "Ogma.swift: custom voice dialog enables paste (regular activation)" \
    "yes" "$(awk '/func customVoice/,/^    }/' "$SCRIPT_DIR/Ogma.swift" | grep -q 'setActivationPolicy(.regular)' && echo "yes" || echo "no")"

check "Ogma.swift: dialogs restore accessory policy via defer" \
    "yes" "$(grep -c 'defer.*setActivationPolicy(.accessory)' "$SCRIPT_DIR/Ogma.swift" | awk '{print ($1 >= 2) ? "yes" : "no"}')"

# API key validation
check "Ogma.swift: validateAPIKey function exists" \
    "yes" "$(grep -q 'func validateAPIKey' "$SCRIPT_DIR/Ogma.swift" && echo "yes" || echo "no")"

check "Ogma.swift: validates key via /v1/user/subscription" \
    "yes" "$(awk '/func validateAPIKey/,/^    }/' "$SCRIPT_DIR/Ogma.swift" | grep -q 'v1/user/subscription' && echo "yes" || echo "no")"

check "Ogma.swift: showAPIKeyDialog calls validateAPIKey" \
    "yes" "$(awk '/func showAPIKeyDialog/,/^    }/' "$SCRIPT_DIR/Ogma.swift" | grep -q 'validateAPIKey' && echo "yes" || echo "no")"

check "Ogma.swift: showAPIKeyDialog loops on validation failure" \
    "yes" "$(awk '/func showAPIKeyDialog/,/^    }/' "$SCRIPT_DIR/Ogma.swift" | grep -q 'while true' && echo "yes" || echo "no")"
...
section "Audio queue player (ogma-audio play-queue)"
...
_TEST_WAV="${TMPDIR:-/tmp}/ogma_test_$$.wav"
python3 -c "
...
# Compile ogma-audio (reuse for queue player tests)
_QP_BIN="${TMPDIR:-/tmp}/ogma_audio_test_$$"
_QP_COMPILED=false
if ! $FAST; then
    if xcrun swiftc "$SCRIPT_DIR/ogma-audio.swift" -o "$_QP_BIN" -O 2>/dev/null; then
        _QP_COMPILED=true
        check "ogma-audio compiles with play-queue support" "true" "$_QP_COMPILED"
    else
...
if $_QP_COMPILED; then
    ...
    _QP_STATUS2="${TMPDIR:-/tmp}/ogma_qpstatus2_$$"
    _run_with_timeout 5 bash -c \
...
rm -f "$_TEST_WAV"
[ "$_QP_BIN" != "$HOME/.local/bin/ogma-audio" ] && rm -f "$_QP_BIN"
...
# Ogma.swift: sentencePause config field
check "Ogma.swift: sentencePause config field" \
    "yes" "$(grep -q 'var sentencePause.*Int.*= 400' "$SETTINGS_SWIFT" && echo "yes" || echo "no")"

check "Ogma.swift: SENTENCE_PAUSE in config load" \
    "yes" "$(grep -q 'case "SENTENCE_PAUSE"' "$SETTINGS_SWIFT" && echo "yes" || echo "no")"

check "Ogma.swift: SENTENCE_PAUSE in config save" \
    "yes" "$(grep -q 'SENTENCE_PAUSE=.*sentencePause' "$SETTINGS_SWIFT" && echo "yes" || echo "no")"

check "Ogma.swift: editSentencePause handler" \
    "yes" "$(grep -q 'func editSentencePause' "$SETTINGS_SWIFT" && echo "yes" || echo "no")"

check "Ogma.swift: Sentence Pause menu item shows current value" \
    "yes" "$(grep -q 'Sentence Pause.*sentencePause' "$SETTINGS_SWIFT" && echo "yes" || echo "no")"

check "Ogma.swift: Sentence Pause uses text input dialog" \
    "yes" "$(awk '/func editSentencePause/,/^    \}/' "$SETTINGS_SWIFT" | grep -q 'NSTextField' && echo "yes" || echo "no")"

# ogma-audio.swift: pause_ms field in protocol
check "ogma-audio.swift: maxSplits is 5 (6 fields)" \
    "yes" "$(grep -q 'maxSplits: 5' "$SCRIPT_DIR/ogma-audio.swift" && echo "yes" || echo "no")"

check "ogma-audio.swift: pauseMs in PlayItem" \
    "yes" "$(grep -q 'pauseMs.*Int' "$SCRIPT_DIR/ogma-audio.swift" && echo "yes" || echo "no")"

check "ogma-audio.swift: asyncAfter for pause delay" \
    "yes" "$(grep -q 'asyncAfter.*delay' "$SCRIPT_DIR/ogma-audio.swift" && echo "yes" || echo "no")"

check "ogma-audio.swift: STATUS_FILE written after pause (inside startPlaying)" \
    "yes" "$(grep -q 'let startPlaying' "$SCRIPT_DIR/ogma-audio.swift" && echo "yes" || echo "no")"
