...
#   venv python at ~/.local/share/ogma/venv (installed by install.command)
...
PID_FILE="${TMPDIR:-/tmp}/ogma_tts.pid"
TEXT_FILE="${TMPDIR:-/tmp}/ogma_text"
...
VENV_PYTHON="${VENV_PYTHON:-$HOME/.local/share/ogma/venv/bin/python3}"
normalize_text() {
    local result
    if [ -x "$VENV_PYTHON" ] && \
...
# ── Locate ogma-audio CLI ──────────────────────────────────────
# Used for both the mute check (below) and the audio queue player (later).
_AUDIO_TOOL="$SCRIPT_DIR/ogma-audio"
[ -x "$_AUDIO_TOOL" ] || _AUDIO_TOOL="$HOME/.local/bin/ogma-audio"

# ── Mute check ────────────────────────────────────────────────────
# When launched from Ogma.app, the mute check is done in-process via
# CoreAudio (microseconds). OGMA_MUTE_CHECKED=1 signals this.
# Standalone: ogma-audio CLI (35ms) or osascript fallback (80-500ms).
if [ "${OGMA_MUTE_CHECKED:-}" != "1" ]; then
    ...
    if _is_muted; then
        mute_result=$(osascript -e 'button returned of (display dialog "Your Mac is muted." with title "Ogma" buttons {"Cancel", "Unmute & Play"} default button "Unmute & Play" with icon caution)' 2>/dev/null) || exit 0
...
if [ "$TTS_BACKEND" = "elevenlabs" ]; then
    if [ -z "$ELEVENLABS_API_KEY" ]; then
        osascript -e 'display dialog "ElevenLabs API key not found." & return & return & "Run install.command to store your key, or set the ELEVENLABS_API_KEY environment variable." with title "Ogma" buttons {"OK"} default button "OK" with icon caution'
        exit 1
    fi
fi
...
# Trace helper: high-resolution timestamp to stderr when OGMA_TRACE is set.
_trace() {
    [ -n "$OGMA_TRACE" ] || return 0
    printf 'TRACE %-16s %s\n' "$1" \
...
run_local_tts() {
    ...
    _daemon_request_bg() {
        local _req_out
        _req_out=$(mktemp "${TMPDIR:-/tmp/}ogma_req_XXXXXXXXXX") || return 1
    ...
    TMP_DIR=$(mktemp -d "${TMPDIR:-/tmp/}ogma_tts_XXXXXXXXXX")
...
play_audio() {
    ...
    if [ -n "$_AUDIO_PLAYER_PID" ]; then
        ...
        # Fallback: afplay (when ogma-audio is unavailable)
        local duration
...
run_elevenlabs_tts() {
    ...
    TMP_FILE=$(mktemp "${TMPDIR:-/tmp/}ogma_tts_XXXXXXXXXX")
...
# Falls back to afplay if ogma-audio is unavailable or OGMA_NO_QUEUE_PLAYER is set.
if [ -x "$_AUDIO_TOOL" ] && [ -z "$OGMA_NO_QUEUE_PLAYER" ]; then
    _AUDIO_IN="${TMPDIR:-/tmp}/ogma_ain_$$"
    _AUDIO_OUT="${TMPDIR:-/tmp}/ogma_aout_$$"
...
if [ "$TTS_BACKEND" = "local" ]; then
    ...
    while IFS=$'\t' read -r _OFFSET _SENT_LEN _SENTENCE; do
        ...
        if $_FIRST && [ $_ok -ne 0 ]; then
            osascript -e 'display dialog "Local TTS generation failed." & return & return & "Re-run the Ogma installer to repair the local TTS setup." with title "Ogma" buttons {"OK"} default button "OK" with icon caution'
            exit 1
        fi
        if [ $_ok -eq 0 ]; then
            _trace "wait_start"
    ...
    if $_FIRST; then
        ...
        if [ $CURL_EXIT -ne 0 ] || [ -z "$HTTP_CODE" ] || [ "$HTTP_CODE" = "000" ]; then
            if [ "$TTS_BACKENDS_INSTALLED" = "both" ]; then
                ...
                osascript -e 'display dialog "Could not reach ElevenLabs, and local TTS also failed." & return & return & "The Kokoro model may need to download first — try again while online." with title "Ogma" buttons {"OK"} default button "OK" with icon caution'
                exit 1
            fi
            osascript -e 'display dialog "Could not reach ElevenLabs." & return & return & "Check your internet connection, or install local TTS for offline use." with title "Ogma" buttons {"OK"} default button "OK" with icon caution'
            exit 1
        fi
        ...
        if [ "$HTTP_CODE" = "429" ]; then
            if [ "$TTS_BACKENDS_INSTALLED" = "both" ]; then
                ...
                osascript -e 'display dialog "ElevenLabs quota exceeded, and local TTS also failed." & return & return & "Re-run the Ogma installer to repair the local TTS setup." with title "Ogma" buttons {"OK"} default button "OK" with icon caution'
                exit 1
            fi
            if [ "$(uname -m)" = "arm64" ]; then
                QUOTA_RESULT=$(osascript -e 'button returned of (display dialog "You'\''ve hit your ElevenLabs quota." & return & return & "Install mlx-audio for free local TTS, or upgrade your ElevenLabs plan." with title "Ogma" buttons {"Not Now", "Install Local TTS"} default button "Install Local TTS" with icon caution)' 2>/dev/null || true)
                if [ "$QUOTA_RESULT" = "Install Local TTS" ]; then
                    if bash "$SCRIPT_DIR/install-local.sh" 2>/dev/null; then
                        osascript -e 'display dialog "Local TTS installed and ready." & return & return & "Future requests will fall back to local when ElevenLabs is unavailable." with title "Ogma" buttons {"OK"} default button "OK"' 2>/dev/null
                        rm -f "$TMP_FILE"; TMP_FILE=""
                        if run_local_tts; then
                            play_audio
                            wait_audio
        ...
        if [ "$HTTP_CODE" != "200" ]; then
            SAFE_ERROR=$(cat "$TMP_FILE" 2>/dev/null \
                | head -c 300 \
                | tr -d '\000-\037"\\')
            osascript -e "display dialog \"ElevenLabs API error (HTTP ${HTTP_CODE}):\" & return & return & \"${SAFE_ERROR:-Unknown error}\" with title \"Ogma\" buttons {\"OK\"} default button \"OK\" with icon caution"
            exit 1
        fi
    fi
fi
